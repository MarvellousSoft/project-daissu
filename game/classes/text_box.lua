...
Then, when you want to "start" the textbox, call its :activate() function, it will initialize cursor blinking and stuff.
...
local TextBox = Class{
    ...
    init = function(self, x, y, w, max_char, lines_on_screen, line_total, show_line_num, text_font, accepted_chars, color, text_color, err_color)
        ...
        Element.init(self)
        Pos.init(self, x, y)
        Clr.init(self, color or Color.new(0, 0, 10, 80, 'HSL'))
        self.w, self.h = w, self.lines_on_screen * self.line_h

        self.text_color = text_color or Color.green('HSL')
        self.err_color = err_color
        if not self.err_color then
            self.err_color = Color.red('HSL')
